...
def train():
    ...
    net = models.M2Det(phase="train", 
                        num_scales=6, 
                        num_levels=8, 
                        num_classes=data.coco["num_classes"],
                        model_name=models.model_names[1])
    net = net.train()
    net = net.to(device)

    dataset = data.COCODetection(root="/home/mcmas/data/coco2017", image_set='train2017', transform=utils.SSDAugmentation(data.coco["min_dim"], net.settings["mean"], net.settings["std"]))
