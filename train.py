...
import layers.modules.multibox_loss as multibox_loss
...
def train():
    ...
    #net.load_state_dict("m2det320_coco_20000.pth")
    net.to(device)
    print(data.COCO_ROOT)
    dataset = data.COCODetection(root="/home/mcmas/data/coco2017", image_set='train2017', transform=augmentations.SSDAugmentation(data.coco["min_dim"], net.settings["mean"], net.settings["std"]))
    ...
    criterion = multibox_loss.MultiBoxLoss(num_classes=data.coco["num_classes"], overlap_thresh=0.5, prior_for_matching=True, bkg_label=0, neg_mining=True, neg_pos=3, neg_overlap=0.5, encode_target=False, use_gpu=True)
